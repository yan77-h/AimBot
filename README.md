...
3. **默认操作**：
   ...
   - 程序自动对屏幕中央 `1440x1440` 区域进行推理。
   - 当检测到 `类别 0` 且距离准星最近的敌人时，鼠标会自动向其移动。
   - **按下 `F12` 键**即可安全退出程序。
   - 默认关闭 OpenCV 预览窗口，需要调试画面时在 `main.py` 中把 `SHOW_PREVIEW` 改为 `True`。
...
| 变量名 | 默认值 | 说明 |
| :--- | :--- | :--- |
| `size` | `1440` | 屏幕捕获区域的边长 (像素) |
| `img_size` | `1440` | 推理输入尺寸；`.engine` 固定为 1440，`.pt` 可尝试调小到 640/800 |
| `fps` | `60` | 目标捕获帧率 |
| `fov` | `300` | 最大响应范围 (距离屏幕中心的半径) |
| `_class` | `0` | 要锁定的目标类别 ID (0为head，1为body) |
| `min_conf` | `0.7` | 最低置信度阈值 (0~1) |
| `smooth_factor` | `0.8` | 平滑系数 (越小移动越慢/越平滑) |
| `SHOW_PREVIEW` | `False` | 是否显示 OpenCV 调试窗口；开启会显著增加延迟 |
| `PRINT_PERF` | `False` | 是否每 60 帧打印一次耗时统计 |
| `DEADZONE` | `1.0` | 鼠标移动死区（像素），避免目标在准星附近时频繁微调 |
| `MAX_DET` | `20` | 推理最多保留的检测框数量，降低后处理开销 |

> **性能提示**：默认关闭预览和性能打印；若使用 `.engine`，`size` 和 `img_size` 应保持 `1440` 以避免额外预处理。若改用 `.pt` 且追求更低延迟，可尝试 `img_size=640` 或 `img_size=800`。
